...
{
  0x0c,   // length of this data( name +1) 
...
// GAP GATT Attributes
//static uint8 attDeviceName[GAP_DEVICE_NAME_LEN] = "Yuenjee Gate";
#define DEFAULT_DEVICE_NAME  "Yuenjee Gate"
...
uint8 Lock_Passcode[70];  //Lock_Passcode
...
/*
自动上报温度定时功能  update_time_s， 0表示不上报，单位 ms, 默认值TEMP_DEFAULT_UPDATE_TIME_MS
*/
...
void SimpleBLEPeripheral_Init( uint8 task_id )
{
  ...
  // Setup the GAP Bond Manager
  {
    ...
    uint8 pairMode = GAPBOND_PAIRING_MODE_NO_PAIRING;//GAPBOND_PAIRING_MODE_INITIATE;//
    uint8 mitm = TRUE;
...
uint8 simpleProfileReadConfig(uint16 uuid, uint8 *newValue)
{
    ...
    switch ( uuid )
    {
      ...
      case SIMPLEPROFILE_CHAR9_UUID:
        HalLcdWriteString( "Char 9 read ADC", HAL_LCD_LINE_3 );
...
static void simpleProfileChangeCB( uint8 paramID )
{
  ...
  switch( paramID )
  {
    ...
    case SIMPLEPROFILE_CHAR6:  // 这个是我们添加char6， 用于做串口透传与从机 notify 数据到主机 很合适
    ...
    case SIMPLEPROFILE_CHAR7: // 收到 CHAR7 的数据
      ...
      HalLcdWriteString( "Char7 Set Name", HAL_LCD_LINE_4 );
      // 需要重启后设备名生效
      break;
    ...
    case SIMPLEPROFILE_CHAR8: // 收到 CHAR8 的数据
	  ...
	  #if (defined HAL_LCD) && (HAL_LCD == TRUE)
        HalLcdWriteStringValue( "Code[0]:", (uint16)(newValue[0]), 16,  HAL_LCD_LINE_5 );
        HalLcdWriteStringValue( "Code[1]:", (uint16)(newValue[1]), 16,  HAL_LCD_LINE_6 );
        HalLcdWriteStringValue( "Code[2]:", (uint16)(newValue[2]), 16,  HAL_LCD_LINE_7 );
        HalLcdWriteStringValue( "Code[3]:", (uint16)(newValue[3]), 16,  HAL_LCD_LINE_8 );
      #endif // (defined HAL_LCD) && (HAL_LCD == TRUE)
      if(returnBytes == 4)
      {
         // Lock_Passcode[0] = newValue[0];
         // Lock_Passcode[1] = newValue[1];
         // Lock_Passcode[2] = newValue[2];
         // Lock_Passcode[3] = newValue[3];           
    ...
    if (events & (HAL_UART_RX_TIMEOUT | HAL_UART_RX_FULL))   //串口有数据
    {
        ...
        if(numBytes > 0)
        {
            ...
            if(buffer)
            {
				...
				//HalLcdDisplayPercentBar( "BLE char1 get npi:",buffer);
				//HalLcdWriteStringValue( "BLE char1 get npi:",buffer, 16, HAL_LCD_LINE_4 );
				//SimpleProfile_SetParameter( SIMPLEPROFILE_CHAR1, sizeof(uint8), &buffer);
